...
STOCK_NUM_FILE="/home/linjc/.config/stock_num.dat"
ITEM_COLOR="\033[34m"
COLOR_RESET="\033[0m"
COLOR_RED="\033[31m"
COLOR_GREEN="\033[32m"
...
function prase_info()
{
	...
	s_name=$(echo ${stock_info_array[1]} | iconv -f gbk -t utf-8)	
...
function cmp_display_rate()
{
	...
	if [ $cmp_value -eq "0" ]
	then
		echo -e "$COLOR_GREEN `echo $1 | awk '{printf("%03d",$0)}'` ($(print_pre_zero $(echo $1 - $2 | bc -l)) $(print_pre_zero $cmp_rate)%)$COLOR_RESET"
	else
		echo -e "$COLOR_RED `echo $1 | awk '{printf("%03d",$0)}'` (+$(print_pre_zero $(echo $1 - $2 | bc -l)) $(print_pre_zero $cmp_rate)%)$COLOR_RESET"
	fi
}
...
function display_info()
{
	#echo -e "$s_date"
	#echo -e "$s_time"
	echo -e "$s_name:$s_num\t $COLOR_RESET $(cmp_display_rate $s_current_price $s_lastday_close)\t $COLOR_RESET $(cmp_display $s_today_open $s_lastday_close)\t $COLOR_RESET $s_lastday_close\t $COLOR_RESET $(cmp_display $s_today_lowest $s_lastday_close)\t $COLOR_RESET $(cmp_display $s_today_highest $s_lastday_close)"
...
{
	echo -e "Ticker \t\t\t   Last \t\t\t  Open \t\t  Close \t   Low \t\t  High"
	echo -e "-------------------------------------------------------------------------------------------------------------------"
}

function display_hs_index()
{
	eval $(echo $1 | awk '{split($0,array,","); for(i in array) print "hs_info_array["i"]="array[i]}' | iconv -f gbk -t utf-8)
